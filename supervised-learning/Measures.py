...
class PredictMove:
  ...
  def train(self, data_loader, model, n, strategy):
    ...
    while accuracy < 0.95 and epoch < self.args["max_epochs"]:
      epoch += 1
      loss_val = self.train_epoch(data_loader, model, optimizer, loss_fn, epoch)
      accuracy = self.test(data_loader, model)
    
      if epoch % 100 == 0:
        print(f"Strategy: {strategy}, size: {n}, epoch: {epoch}, loss: {loss_val:.4f}, accuracy: {accuracy:.4f}")
  ...
  def run_one(self, n, strategy):
    ...
    if os.path.exists(model_path):
      ...
      epoch, ce = self.train(data_loader, model, n, strategy)
      state_dict = dict()
    ...
    return epoch, ce, accuracy
  
  def run_all(self):
    for strategy in self.args["to_train"]:
      tmp_results = []
      for n in range(self.args["min_n"], self.args["max_n"]+1):
        epoch, ce, accuracy = self.run_one(n, strategy)
        tmp_results.append((n, epoch, ce, accuracy))
        print(f"{strategy} - Size: {n}, epochs: {epoch},  loss: {ce:.4f}, accuracy: {accuracy:.4f}")
  ...
  def save_results(self):
    ...
    for strategy, result in zip (self.args["to_train"], self.results):
      for size, epoch, ce, acc in result:
        to_df.append([strategy, size, epoch, ce, acc, self.args["experiment_label"]])
    df = pd.DataFrame(to_df, columns=["strategy", "size", "epochs", "ce", "accuracy", "network"])
